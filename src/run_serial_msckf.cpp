...
int main(int argc, char** argv)
{
    ...
    builder = new MapBuilder(nh, sys, params);
    ...
    for (const rosbag::MessageInstance& m : view) {
        ...
        if (s2 != NULL && m.getTopic() == topic_imu) {
            ...
            builder->feed_measurement_imu(timem, wm, am);
        }
        ...
        if(max_cameras==1 && has_left) {
            ...
            // visualize
            viz->visualize();
            // append to map builder
            std::vector<size_t> cam_ids = {0};
            std::vector<cv::Mat> images;
            images.push_back(img0_buffer);
            builder->feed_measurement_camera(time_buffer, images, cam_ids);
            // reset bools
            has_left = false;
            // move buffer forward
            time_buffer = time;
        ...
        if(max_cameras==2 && has_left && has_right) {
            ...
            viz->visualize();
            // append to map builder
            std::vector<size_t> cam_ids = {0,1};
            std::vector<cv::Mat> images;
            images.push_back(img0_buffer);
            images.push_back(img1_buffer);
            builder->feed_measurement_camera(time_buffer, images, cam_ids);
            // reset bools
            has_left = false;
            has_right = false;
